...
void delay_us(uint16_t us) {  //因為函式庫內 delay(), unit為毫秒ms，因此需自定義微秒延遲器
    __HAL_TIM_SET_COUNTER(&htim3, 0);  // 設定計數器歸零
    while (__HAL_TIM_GET_COUNTER(&htim3) < us); // while持續進行直到計數器= us
}

// 發送 Trig 信號
void HCSR04_Trigger() {
    HAL_GPIO_WritePin(TRIG_GPIO_Port, TRIG_Pin, GPIO_PIN_SET);  // 高電平
    delay_us(10);  // 保持 10us
    HAL_GPIO_WritePin(TRIG_GPIO_Port, TRIG_Pin, GPIO_PIN_RESET);  // 低電平
}
